...
if len(sys.argv) > 3:
    ...
    print(current_dir)
    input_path = os.path.join(current_dir,"predectionbucket\data\csv\testdata.csv")
    model_path= os.path.join(current_dir, "predectionbucket\data\model\testdata.model")
df = (spark.read
        .format("csv")
        .option('header', 'true')
        .option("sep", ";")
